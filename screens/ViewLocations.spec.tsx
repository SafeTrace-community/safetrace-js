...
describe('Location logs screen', () => {
    ...
    test('TEMPORARILY reversing the ordering of logs', async () => {
        const locations = [
            {
                coords: {
                    speed: -1,
            ...
            },
            {
                coords: {
                    ...
                    heading: -1,
                    accuracy: 65,
                    altitude: 19.169458389282227,
                    latitude: 51.53023022272163,
                    longitude: -0.09415571517543876,
                    altitudeAccuracy: 10,
                },
                timestamp: 1587652645563.399,
            },
        ];
        ...
        await act(async () => {
            await wait(() => expect(getAllByTestId('log')).toHaveLength(2));

            const firstLogListItem = getAllByTestId('log')[0];

            expect(
                within(firstLogListItem).getByTestId('latitude').props.children
            ).toEqual(51.53023022272163);
            expect(
                within(firstLogListItem).getByTestId('longitude').props.children
            ).toEqual(-0.09415571517543876);
            expect(
                within(firstLogListItem).getByTestId('altitude').props.children
            ).toEqual(19.169458389282227);

            const secondLogListItem = getAllByTestId('log')[1];

            expect(
                within(secondLogListItem).getByTestId('latitude').props.children
            ).toEqual(51.5302747697369);
            expect(
                within(secondLogListItem).getByTestId('longitude').props
                    .children
            ).toEqual(-0.09434700051002931);
            expect(
                within(secondLogListItem).getByTestId('altitude').props.children
            ).toEqual(19.153566360473633);
        });
    });
